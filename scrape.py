...
(args) => {
  ...
  for (const el of document.querySelectorAll('div,span,li,a,p,button,label,h1,h2,h3,h4')) {
    ...
    if (s > bestScore || (s === bestScore && t.length < bestLen)) {
      best = el; bestScore = s; bestLen = t.length;
    }
  }
  if (!best) return null;
  // climb to the tightest card container that carries a $ price, then take the
  // largest $ figure (the buyer price; excludes the "per $1" ratio's "$1"). If
  // the matched card is sold out (no $ price), report that so we record N/A
  // instead of mistaking the face value ("100 USD") for a price.
  // NB: avoid bare "unavailable" — Eneba shows "unavailable in United States"
  // as a region notice on items that still list prices; that's not sold out.
  const soldRe = /sold\s*out|out of stock|currently unavailable|temporarily unavailable|notify me when/i;
  let node = best;
  for (let i = 0; i < 6 && node; i++) {
    const txt = node.textContent || '';
    const vals = [...txt.matchAll(/\$\s?(\d[\d.,]*)/g)]
      .map(m => parseFloat(m[1].replace(/,/g, ''))).filter(v => !isNaN(v) && v > 0);
    if (vals.length) return { price: Math.max(...vals) };
    if (soldRe.test(txt) && txt.length < 200) return { soldout: true };
    node = node.parentElement;
  }
  return { soldout: soldRe.test(best.textContent || '') };
}
"""
...
async def adapter_eneba(page, url, denom):
    ...
    try:
        txt = await page.evaluate(r"""
        () => {
          // Buy-button price: a $-prefixed amount only (avoid bare "100 USD"
          // face values, which aren't prices).
          const re = /(US\$|\$)\s?\d[\d.,]*/;
    ...
    if txt:
        ...
        if pc:
            return pc
    # Nothing priced. If the page/denomination is sold out, record N/A; otherwise
    # report nothing (do NOT generic-scan — it grabs Eneba face values as prices).
    try:
        body = await page.inner_text("body")
    except Exception:
        body = ""
...
def _og_offer_price(o):
    """The price a buyer actually pays = the lowest of the offer's price-like
    fields (the discounted/selling price, e.g. 392 rather than the 400 list
    price). OG returns the discount under a separate *_price key, so taking the
    min of all USD price fields picks the discounted figure."""
    vals = []
...
def fetch_og_price_sync(catalog_url, pname=""):
    """Resolve an OG price without trusting the (rotating) offer_id: pull the live
    offer list for the product and match the denomination by title. Returns the
    discounted price even when out of stock (per requirement), or None.
    Prefers an in-stock offer's price; falls back to any matched offer's price."""
    ...
    if seo and want["nums"]:
        ...
        if sid and bid:
            ...
            cols = ((data or {}).get("payload") or {}).get("results") or []
            instock, allprices = [], []
            for col in cols:
                for o in (col.get("offer_results") or []):
                    if not _og_match(want, _og_sig(o.get("title"))):
                        continue
                    price = _og_offer_price(o)
                    if getattr(ARGS, "debug", False):
                        # dump the WHOLE matched offer so we can see exactly which
                        # field holds the after-discount price / discount percent.
                        try:
                            dump = json.dumps(o, ensure_ascii=False)[:1200]
                        except Exception:
                            dump = str(o)[:1200]
                        print(f"    [OG-DEBUG] {pname!r} :: {o.get('title')!r} "
                              f"qty={o.get('available_qty')} chosen={price}\n"
                              f"      OFFER={dump}", flush=True)
                    if price is None:
                        continue
                    allprices.append(price)
                    qty = o.get("available_qty")
                    if isinstance(qty, (int, float)) and qty > 0:
                        instock.append(price)
            if instock:
                return (round(min(instock), 4), "USD")
            if allprices:    # out of stock, but still report the price
                return (round(min(allprices), 4), "USD")

    # Fallback: the direct offer_id endpoint (works while the id is still valid).
    q = _uparse.parse_qs(_uparse.urlparse(catalog_url).query)
    ...
    if oid:
        ...
        price = _og_offer_price(p)            # discounted; report even if OOS
        if getattr(ARGS, "debug", False):
            pf = {k: v for k, v in p.items()
                  if "price" in k.lower() and isinstance(v, (int, float))}
            print(f"    [OG-DEBUG offer_id] {pname!r} qty={p.get('available_qty')} "
                  f"chosen={price} price_fields={pf}", flush=True)
        if price is None:
