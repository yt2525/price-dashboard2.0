...
async def adapter_seagm(page, url, denom):
    ...
    # name and read the ORANGE discounted price (a <b>; .price_origional is the
    # crossed-out original, so exclude it). SEAGM is pinned to USD for the whole
    # run (see seagm_force_usd), so the price is USD; if the pin ever fails,
    # to_usd() converts the regional price rather than leaving the cell blank.
