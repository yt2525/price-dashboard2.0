...
def build_tasks(data, only_categories, limit, only_comps=None, skip_comps=None,
    ...
    for cat, block in data["categories"].items():
        ...
        for product in block["products"]:
                ...
                for comp, info in product["urls"].items():
                    ...
                    if isinstance(u, str) and u.startswith("http"):
                        tasks.append({"sheet": cat, "pname": product["name"], "comp": comp,
                                      "url": u, "note": (info.get("note") if isinstance(info, dict) else None),
            ...
            # --- NORMAL run: existing catalogue products only ------------------
            # Custom products are skipped here (priced via --custom-search instead),
            # so the regular scrape stays fast and reliable.
            if is_custom:
                continue
            for comp, info in product["urls"].items():
                if not comp_ok(comp):
