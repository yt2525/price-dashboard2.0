...
async def adapter_seagm(page, url, denom):
    # All denominations are on one product page. Match by denomination number +
    # name and read the orange final price (a <b>; .price_origional is the
    # crossed-out original, so exclude it). SEAGM is pinned to USD for the whole
    # run (see seagm_force_usd), so the price is USD and to_usd() is a no-op; if
    # the pin ever fails it converts the regional price rather than going blank.
...
def usd_url(url):
    # SEAGM: currency is a cookie set once per run to USD (see seagm_force_usd).
    # IMPORTANT: visiting an /en-my/ URL RESETS the currency back to MYR, so we
    # must keep every SEAGM URL on /en-us/, which preserves the USD cookie and
    # shows US$ prices. (Verified live.)
    host = urlparse(url).netloc.replace("www.", "").lower()
    if host.endswith("seagm.com"):
        p = urlparse(url)
        parts = [x for x in p.path.split("/") if x]
        if parts and re.fullmatch(r"[a-z]{2}-[a-z]{2}", parts[0]):
            parts[0] = "en-us"               # replace existing locale (incl. en-my)
        else:
...
async def seagm_force_usd(context):
    ...
    page = await context.new_page()
    try:
        await page.goto("https://www.seagm.com/en-us/language_currency",
                        timeout=NAV_TIMEOUT_MS, wait_until="domcontentloaded")
        await page.wait_for_timeout(1500)
        # Set currency=USD and submit the form via JS (this exact approach is
        # verified to flip SEAGM's prices; Playwright select_option + button
        # click did NOT take in headless).
        ok_submit = await page.evaluate(r"""
        () => {
          const c = document.querySelector('select[name="currency"]');
          if (!c) return false;
          c.value = 'USD';
          c.dispatchEvent(new Event('change', { bubbles: true }));
          const lang = document.querySelector('select[name="language"]');
          if (lang) { lang.value = 'en'; lang.dispatchEvent(new Event('change', { bubbles: true })); }
          const f = c.closest('form');
          if (f) { f.submit(); return true; }
          return false;
        }
        """)
        print(f"  [SEAGM] currency form submitted: {ok_submit}", flush=True)
        await page.wait_for_timeout(3000)
        # verify on a product page
        await page.goto("https://www.seagm.com/en-us/steam-wallet-card-code-global",
                        timeout=NAV_TIMEOUT_MS, wait_until="domcontentloaded")
