...
def fetch_g2g_price_sync(catalog_url):
    ...
    for ctx in (None, ssl._create_unverified_context()):
        try:
            ...
            if prices:
                return (min(prices), "USD")
            # API responded but there are no offers for this denomination -> sold out.
            return NA
        except Exception:
            continue
    return None
...
# longer codes first so "USDT"/"USDC" win over "USD" in the alternation
_OG_DENOM_RE = re.compile(
    r"(USDT|USDC|USD|EUR|GBP|JPY|MYR|SGD|AUD|CAD|CNY|TRY|BRL|IDR|PHP|HKD|TWD|KRW|THB|VND)"
    r"\s*0*(\d[\d.]*)", re.I)


def _og_token(text):
    """('USDT', 50.0) from 'Binance Gift Card USDT50' or 'USDT 50'."""
    if not text:
        return None
    m = _OG_DENOM_RE.search(str(text).replace(",", ""))
    if not m:
        return None
    try:
...
def fetch_og_price_sync(catalog_url, pname=""):
    ...
    offer list for the product and match the denomination by title. Returns
    (usd, 'USD'), NA (denomination exists but out of stock), or None."""
    seo = _og_seo_term(catalog_url)
    want = _og_token(pname)
    if seo and want:
        ...
        if sid and bid:
            ...
            instock, matched = [], False
            for col in cols:
                for o in (col.get("offer_results") or []):
                    tok = _og_token(o.get("title"))
                    if not tok or tok[0] != want[0] or tok[1] != want[1]:
                        continue
                    matched = True
                    price = o.get("converted_unit_price")
            ...
            if matched:
                return NA   # denomination found but every offer is out of stock
...
async def adapter_g2a(page, url, denom):
    # G2A server-renders every offer into a <script type="application/json+redux">
    # blob in the raw HTML. We re-fetch the raw HTML (the page's own session beats
    # Cloudflare, and React strips the blob from the live DOM after hydration), walk
    # sections.data[*].data.offers, and take the lowest prices.normal.price (the
    # buyer total). app.currency tells us the currency for conversion. Each G2A URL
