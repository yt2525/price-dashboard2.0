...
_SEAGM_JS = r"""
(args) => {
  // Every denomination is a `div.SKU_type` card, e.g. text "PSN Card 250 USD US"
  // followed by `<div class="price"><b>US$ 231.25</b><b class="price_origional">
  // US$ 250.00</b></div>`. We match the card by the denomination number (+ name
  // words) against the NAME ONLY (the text before the first "US$", so price
  // digits never pollute the match), then read the ORANGE discounted price — the
  // <b> that is NOT .price_origional. USD only: the site already prints US$, so
  // we never convert. If a card has no US$ price (session isn't USD), we report
  // that instead of grabbing a regional number.
  ...
  let cards = [...document.querySelectorAll('div.SKU_type')];
  if (!cards.length) {
    // fallback: climb from each price block to a small labelled container
    cards = [...document.querySelectorAll('div.price')].map(p => {
      let n = p; for (let i = 0; i < 4 && n.parentElement; i++) n = n.parentElement; return n;
    });
  }
  ...
  for (const c of cards) {
    ...
    let s = 0; for (const re of numRes) if (re.test(name)) s += 10;
    const nl = name.toLowerCase(); for (const w of words) if (nl.includes(w)) s += 1;
    if (numRes.length && s < 10) continue;       // denomination number must match
    if (s === 0) continue;
...
(args) => {
  ...
  for (const el of document.querySelectorAll('div,span,button,label')) {
    ...
    if (t.length < 16 && /^\d[\d.]*\s*[A-Z]{2,4}$/.test(t)) faces.add(t);
  }
    ...
    for c in cands:
        ...
        for n in nums:
            if re.search(r"(?<!\d)" + str(n) + r"(?!\d)", t):
                s += 10
        if nums and s == 0:
            continue                                # denomination number must match
        for w in words:
            if w in tl:
                s += 1
        if region and re.search(r"\b" + re.escape(region) + r"\b", t, re.I):
            s += 3                                   # prefer the matching region/currency
        cheaper = (s == best_score and best and c.get("price") and best.get("price")
                   and c["price"] < best["price"])
        if s > best_score or cheaper:
            best, best_score = c, s
    # require a real denomination match (score >= 10) when the product has a number
    if not best or (nums and best_score < 10):
        return None
    url = best["url"]
    if url.startswith("/"):
        url = urljoin(site["search"](""), url)
...
def usd_url(url):
    ...
    if host.endswith("seagm.com"):
        ...
        if parts and re.fullmatch(r"[a-z]{2}-[a-z]{2}", parts[0]):
            parts = parts[1:]                # strip leading locale (en-my / en-us)
...
async def fetch_one(context, task, results, report):
    ...
    if task.get("search") and not url:
        ...
        try:
            ...
            resolved = await resolve_via_search(rpage, comp, task.get("query") or task["pname"], denom0)
        except Exception:
            resolved = None
        finally:
            try:
...
def build_tasks(data, only_categories, limit, only_comps=None, skip_comps=None,
    ...
    for cat, block in data["categories"].items():
        ...
        for product in block["products"]:
            ...
            # Search fallback: for competitors with NO stored URL on this product,
            # add a search task (only for comps we know how to search + were asked
            # to via --search-comps, and that pass the comp filters).
            for comp in (search_comps or set()):
                if comp in present or comp in SKIP_COMPS or comp not in SEARCH_SITES:
                    continue
                if only_comps and comp not in only_comps:
                    continue
                if skip_comps and comp in skip_comps:
