...
def usd_url(url):
    # SEAGM: use the /en-my/ link. Currency is set to USD once per run via the
    # settings form ON en-my (see seagm_force_usd). The locale of the currency
    # switch MUST match the product URLs (mixing en-us/en-my resets currency to
    # MYR), so keep everything on /en-my/ — verified to show US$ prices.
    host = urlparse(url).netloc.replace("www.", "").lower()
    if host.endswith("seagm.com"):
        p = urlparse(url)
        parts = [x for x in p.path.split("/") if x]
        if parts and re.fullmatch(r"[a-z]{2}-[a-z]{2}", parts[0]):
            parts[0] = "en-my"               # replace existing locale (incl. en-us)
        else:
