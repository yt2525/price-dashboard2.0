...
(args) => {
  ...
  for (const el of document.querySelectorAll('div,span,li,a,p,button,label,td,h1,h2,h3,h4')) {
    ...
    if (!/USD|EUR|GBP|JPY|CNY|CAD|MYR|RM|SGD|QAR|AUD|USDT|USDC|Diamond|Crystal|Genesis|Point|Coin|Gold|Credit|Shard|Lunite|Gem|Rbx|Oneiric|Monochrome|Bonus|Welkin|Pass|Nitro|Premium|Wallet|Day|Days|Month|Months|Year|Years/i.test(t)) continue;            // titles only, not the whole grid
...
(args) => {
  ...
  for (const el of document.querySelectorAll('div,span,li,a,p,button,label,td,h1,h2,h3,h4')) {
    const t = (el.textContent || '').replace(/,/g, '');
    if (t.length > 90) continue;
    if (!/USD|EUR|GBP|JPY|CNY|CAD|MYR|RM|SGD|QAR|AUD|USDT|USDC|Diamond|Crystal|Genesis|Point|Coin|Gold|Credit|Shard|Lunite|Gem|Rbx|Oneiric|Monochrome|Bonus|Welkin|Pass|Nitro|Premium|Wallet|Day|Days|Month|Months|Year|Years/i.test(t)) continue;
    let score = 0; for (const re of numRes) if (re.test(t)) score++;
    if (score === 0) continue;
    if (score > bestScore || (score === bestScore && t.length < bestLen)) {
      label = el; bestScore = score; bestLen = t.length;
