...
def fetch_og_price_sync(catalog_url, pname=""):
    ...
    if seo and want["nums"]:
        ...
        if sid and bid:
            # search/lite gives us the legacy numeric category id (lgc_category_id);
            # its converted_unit_price is the SRP, so we don't price off it.
            data = _og_get_json(
                "https://sls.offgamers.com/offer/search/lite?"
                + _uparse.urlencode({"service_id": sid, "brand_id": bid,
                                     "country": "US", "currency": "USD"}))
            cat_id = _og_find_cat_id(data)
        # getProductList holds the DISCOUNTED price (raw_final_price) per denom.
        if cat_id:
            pl = _og_get_json("https://www.offgamers.com/site/getProductList?"
                              + _uparse.urlencode({"cat_id": cat_id, "currency": "USD"}))
