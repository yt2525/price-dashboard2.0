...
(args) => {
  ...
  for (const el of document.querySelectorAll('div,span,li,a,p,button,label,td,h1,h2,h3,h4')) {
    ...
    // numScore counts matched target numbers; maxNum is the LARGEST one matched.
    // For base+bonus packs (e.g. "600 + 60 UC", nums [600,60,660]) several cards
    // tie on count — the "60 UC" card and the "660 UC" total both match one
    // number. We must pick the largest matched number (660, the real total) and
    // NOT fall back to the shorter label, which would wrongly grab the $1 "60 UC".
    let numScore = 0, maxNum = -1;
    for (let i = 0; i < numRes.length; i++) {
      if (numRes[i].test(t)) { numScore++; if (nums[i] > maxNum) maxNum = nums[i]; }
    }
...
(args) => {
  ...
  const scoreName = (name) => {
    ...
    let s = 0; for (const re of numRes) if (re.test(nm)) s += 10;
...
async def adapter_kinguin(page, url, denom):
    # PRIMARY: read the embedded _preloadedState (reliable; survives DOM changes).
    # The URL's numeric categoryId pins the product, so lowestPrice = this
    # denomination's cheapest offer. Currency is whatever the page renders in
    # (geo-dependent on a residential IP) — we read it and let to_usd convert.
    try:
    ...
    if data:
        ...
        if data.get("oos"):
            return NA
    # FALLBACK: the live price in the main offer section (data-test is stable).
