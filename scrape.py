...
async def seagm_force_usd(context):
    ...
    try:
        ...
        try:
            await page.select_option('select[name="currency"]', "USD", timeout=5000)
        except Exception as e:
            print(f"  [SEAGM] currency select failed: {type(e).__name__}", flush=True)
        try:
            await page.select_option('select[name="language"]', "en", timeout=2000)
        except Exception:
            pass
        # submit the "Save Settings" form
        for sel in ['input[type="submit"][value*="Save"]', 'input.button[type="submit"]',
                    'input[type="submit"]', 'text=Save Settings']:
            try:
