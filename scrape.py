...
def usd_url(url):
    # SEAGM: use the MY link (/en-my/). Currency is a separate cookie pinned to
    # USD once per run (see seagm_force_usd), so /en-my/ pages still show US$
    # prices. Normalise every SEAGM URL to the /en-my/ locale.
    host = urlparse(url).netloc.replace("www.", "").lower()
    if host.endswith("seagm.com"):
        p = urlparse(url)
        parts = [x for x in p.path.split("/") if x]
        if parts and re.fullmatch(r"[a-z]{2}-[a-z]{2}", parts[0]):
            parts[0] = "en-my"               # replace existing locale
        else:
            parts = ["en-my"] + parts        # insert locale
...
async def seagm_force_usd(context):
    ...
    page = await context.new_page()
    try:
        await page.goto("https://www.seagm.com/en-my/language_currency",
        ...
        await page.goto("https://www.seagm.com/en-my/steam-wallet-card-code-global",
                        timeout=NAV_TIMEOUT_MS, wait_until="domcontentloaded")
