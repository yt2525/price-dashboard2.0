...
async def seagm_force_usd(context):
    ...
    currency (e.g. MYR), so we replicate the manual flow (open the currency
    settings -> select USD -> Save Settings). The cookie then persists, so every
    SEAGM product page in this run prices natively in USD. Best-effort."""
    page = await context.new_page()
    try:
        ...
        try:
            ...
            body = ""
        if "US$" in body:
            print("  [SEAGM] currency already USD", flush=True)
            return
        # open the currency/region settings panel (header currency indicator)
        for sel in [r"text=/\b(MYR|RM|SGD|IDR|PHP|THB|VND|AUD|GBP|EUR)\b/",
                    "[class*=currency]", "[class*=country]", "[class*=locale]"]:
            try:
        ...
        # pick USD in whichever <select> offers it
        sels = page.locator("select")
        for i in range(await sels.count()):
            try:
                await sels.nth(i).select_option("USD", timeout=1500)
                break
            except Exception:
                continue
        # save the setting
        for sel in ["text=Save Settings", "button:has-text('Save')", "text=Save"]:
            try:
        ...
        try:
            ...
            ok = False
        print(f"  [SEAGM] currency switch to USD: {'ok' if ok else 'unconfirmed'}", flush=True)
    except Exception as e:
