...
(args) => {
  ...
  // PRICE ALWAYS WINS: if any $ price is shown for this denomination card, take
  // it even when the card also says "sold out". Only report sold-out (-> N/A)
  // when no price is shown anywhere in the card.
  const soldRe = /sold\s*out|out of stock|currently unavailable|temporarily unavailable|notify me when/i;
  let node = best, sold = false;
  for (let i = 0; i < 6 && node; i++) {
    const txt = node.textContent || '';
    if (i < 4) {                                   // look for a price within the card scope
...
def _og_find_cat_id(obj):
    """Find a numeric category id anywhere in an OG JSON payload."""
    if isinstance(obj, dict):
        for k, v in obj.items():
            if k.lower() in ("cat_id", "category_id", "categories_id", "catid") \
                    and str(v).isdigit():
                return str(v)
        for v in obj.values():
            r = _og_find_cat_id(v)
...
def fetch_og_price_sync(catalog_url, pname=""):
    ...
    `raw_final_price` (the after-discount price). The discounted prices live in
    www.offgamers.com/site/getProductList?cat_id=<id>; we resolve cat_id from
    seo_info. Prefers in-stock offers but still reports a price when out of stock.
    Returns (usd, 'USD') or None."""
    ...
    if seo and want["nums"]:
        ...
        _og_collect_offers(info, candidates)
        # Primary source of the DISCOUNTED price: getProductList for the category.
        cat_id = _og_find_cat_id(info)
        if cat_id:
