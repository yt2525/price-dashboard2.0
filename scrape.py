...
def build_tasks(data, only_categories, limit, only_comps=None, skip_comps=None,
    ...
    for cat, block in data["categories"].items():
        ...
        for product in block["products"]:
            ...
            # add a search task. Existing products only search the conservative
            # --search-comps set; user-added (custom) products search EVERY
            # marketplace we know how to search, since fetch-by-name is their whole
            # point. The denomination-number gate keeps non-matches at N/A.
            searchable = set(SEARCH_SITES) | set(API_SEARCH)
            search_set = set(search_comps or set())
            if product.get("custom"):
                search_set |= searchable
            for comp in search_set:
                if comp in present or comp in SKIP_COMPS or comp not in searchable:
                    continue
                if only_comps and comp not in only_comps:
                    continue
                if skip_comps and comp in skip_comps:
            ...
            for cat, blk in (custom.get("categories") or {}).items():
                ...
                for p in (blk.get("products") or []):
                    ...
                    if not name or name in existing:
                        continue
                    dst["products"].append({"name": name,
                                            "urls": p.get("urls") or {}, "custom": True})
                    existing.add(name)
                    added += 1
            if added:
