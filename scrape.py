...
(args) => {
  ...
  for (const el of document.querySelectorAll('div,span,li,a,p,button,label,h1,h2,h3,h4')) {
    ...
    if (curRe && !curRe.test(t)) continue;       // face currency must match if known
  ...
  // largest $ figure (the buyer price; excludes the "per $1" ratio's "$1").
  let node = best;
  for (let i = 0; i < 6 && node; i++) {
    const txt = node.textContent || '';
    const vals = [...txt.matchAll(/\$\s?(\d[\d.,]*)/g)]
      .map(m => parseFloat(m[1].replace(/,/g, ''))).filter(v => !isNaN(v) && v > 0);
    if (vals.length) return 'USD ' + Math.max(...vals);
    node = node.parentElement;
  }
  return null;
}
...
async def adapter_eneba(page, url, denom):
    # Grid-match the denomination card and read its $ (USD) buyer price. Falls
    # back to the buy-button scan, then the generic text scan.
    try:
        txt = await page.evaluate(_ENEBA_JS, {"nums": denom.get("nums") or [],
    ...
    if txt:
        ...
        if pc:
            return pc
    # Single-offer pages (no value grid, e.g. Nutaku) show one "FEATURED OFFER"
    # price. Grab the $ price from the smallest element mentioning it.
    try:
        ...
        () => {
          ...
          for (const el of document.querySelectorAll('div,section,span,p,strong,b')) {
            ...
            if (!/featured offer/i.test(t)) continue;
            const m = t.match(/\$\s?(\d[\d.,]*\.\d{2})/) || t.match(/\$\s?(\d[\d.,]*)/);
            if (m && t.length < bestLen) { best = m[1].replace(/,/g, ''); bestLen = t.length; }
          }
          return best ? 'USD ' + best : null;
        }
        """)
    except Exception:
        txt = None
    ...
    try:
        ...
        txt = None
    return parse_price_currency(txt) if txt else await generic_extract(page, url, denom)
